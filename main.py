...
class EmbeddingLayer(torch.nn.Module):
    def __init__(self, embed_dim, patch_size):
        ...
        self.conv = torch.nn.Conv2d(in_channels=3, out_channels=embed_dim, kernel_size=patch_size, stride=patch_size)
...
if __name__ == "__main__":
    ...
    for x,y in dataloader_train:
        ...
        optimizer.zero_grad()
        y_hat = model(x)
        l = lossfn(y_hat.to(torch.float32), y.to(torch.long), )
        l.backward()
        optimizer.step()
        print(l)
